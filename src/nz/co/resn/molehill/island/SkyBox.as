...
package nz.co.resn.molehill.island {
    ...
    public class SkyBox extends RenderObject3D {

        protected var cubeMap:CubeTexture;
        protected var SkyBitmap:Class;

        public function SkyBox(context3D:Context3D, bitmapClass:Class){
            super(context3D);
            this.SkyBitmap = bitmapClass;
            this.context3D = context3D;
            this.createSkyCube(8, _vertices, _indencies);
            _indexBuffer = context3D.createIndexBuffer(_indencies.length);
            _indexBuffer.uploadFromVector(_indencies, 0, _indencies.length);
            _vertexBuffer = context3D.createVertexBuffer((_vertices.length / 4), 4);
            _vertexBuffer.uploadFromVector(_vertices, 0, (_vertices.length / 4));
            var skyMapData:ByteArray = (new SkyBitmap() as ByteArray);
            this.cubeMap = context3D.createCubeTexture(0x0200, Context3DTextureFormat.COMPRESSED, false);
            this.cubeMap.uploadCompressedTextureFromByteArray(skyMapData, 0);
            this.initPrograms();
        }
        override protected function initPrograms():void{
            _vertexShaderAssembler = new AGALMiniAssembler();
            _vertexShaderAssembler.assemble(Context3DProgramType.VERTEX, ("m44 op, va0, vc0\n" + "mov v0, va0\n"));
            _fragmentShaderAssembler = new AGALMiniAssembler();
            _fragmentShaderAssembler.assemble(Context3DProgramType.FRAGMENT, ("tex ft0, v0, fs0 <cube,dxt1,linear,clamp,miplinear>\n" + "mov oc, ft0\n"));
